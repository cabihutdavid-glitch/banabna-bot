...
app.post('/chat', authenticate, async (req, res) => {
  ...
  try {
    ...
    let apiMessages = [{ role: 'system', content: `Eres Banabna, un asistente personal. ${user?.name ? 'El usuario se llama ' + user.name + '.' : ''} Responde siempre en espanol.` }];
    if (image) {
...
app.post('/save-session', authenticate, (req, res) => {
  ...
  const userId = req.user.id;
  try { db.sessions.push({ id: Date.now(), user_id: userId, messages: JSON.stringify(messages), created_at: new Date().toISOString() }); saveDB(); res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get('/sessions', authenticate, (req, res) => {
  ...
  res.json(sessions.map(s => ({ id: s.id, preview: JSON.parse(s.messages).find(m => m.role === 'user')?.content?.substring(0, 60) || 'Chat', date: s.created_at })));
});
...
app.post('/translate', authenticate, async (req, res) => {
  ...
  try {
    const completion = await groq.chat.completions.create({ model: 'llama-3.3-70b-versatile', messages: [{ role: 'system', content: 'Translate from ' + from + ' to ' + to + '. Only respond with translation.' }, { role: 'user', content: text }], max_tokens: 2000 });
...
if (require.main === module) { app.listen(PORT, () => console.log('Banabna en http://localhost:' + PORT)); }